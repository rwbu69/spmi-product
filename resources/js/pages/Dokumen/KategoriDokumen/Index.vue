...
import { Edit2, FolderOpen, Plus, Search, Trash2 } from 'lucide-vue-next';
...
watch(search, (val) => {
    clearTimeout(st);
    st = setTimeout(() => router.get('/dokumen/kategori', { search: val }, { preserveState: true, replace: true }), 400);
});

const showForm = ref(false);
const editTarget = ref<Kategori | null>(null);
const form = useForm({ nama_kategori: '' });

const openCreate = () => { editTarget.value = null; form.reset(); form.clearErrors(); showForm.value = true; };
const openEdit = (item: Kategori) => { editTarget.value = item; form.nama_kategori = item.nama_kategori; form.clearErrors(); showForm.value = true; };
...
const submit = () => {
    ...
    editTarget.value
        ? form.put(`/dokumen/kategori/${editTarget.value.id}`, opts)
        : form.post('/dokumen/kategori', opts);
};

const showDelete = ref(false);
const deleteTarget = ref<Kategori | null>(null);
const deleting = ref(false);
const openDelete = (item: Kategori) => { deleteTarget.value = item; showDelete.value = true; };
const closeDelete = () => { showDelete.value = false; deleteTarget.value = null; };
const confirmDelete = () => {
    if (!deleteTarget.value) return;
    deleting.value = true;
    router.delete(`/dokumen/kategori/${deleteTarget.value.id}`, {
        preserveScroll: true,
...
<template>
    <Head title="Kategori Dokumen" />
    <div class="space-y-6 p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-semibold text-gray-900 ">Kategori Dokumen</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola kategori dokumen mutu</p>
            </div>
            <button id="btn-tambah-kategori-dok" type="button"
            ...
            <table class="w-full text-sm">
                <thead class="bg-blue-50 text-blue-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Nama Kategori</th>
                        <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Jumlah Jenis</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr v-for="(item, i) in data.data" :key="item.id"
                        ...
                        <td class="px-4 py-3 text-gray-500">{{ item.jenis_dokumen_count }} jenis</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <button :id="`btn-edit-kat-${item.id}`" type="button"
                                ...
                                <button :id="`btn-delete-kat-${item.id}`" type="button"
    ...
    <Modal :show="showForm" :title="editTarget ? 'Edit Kategori' : 'Tambah Kategori'" max-width="sm" @close="closeForm">
        <form @submit.prevent="submit" class="space-y-4">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Nama Kategori <span class="text-red-500">*</span>
                </label>
                <input v-model="form.nama_kategori" type="text" placeholder="Contoh: KEBIJAKAN MUTU"
                    ...
                    :class="form.errors.nama_kategori ? 'border-red-400' : 'border-gray-300'" />
                <p v-if="form.errors.nama_kategori" class="mt-1 text-xs text-red-500">{{ form.errors.nama_kategori }}</p>
            </div>
    ...
    <ConfirmDeleteModal :show="showDelete" :message="`Hapus kategori '${deleteTarget?.nama_kategori}'?`"
