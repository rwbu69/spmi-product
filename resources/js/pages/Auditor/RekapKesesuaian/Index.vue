...
interface RekapKesesuaian {
    id: number;
    nama_auditee: string;
    jumlah_kesesuaian: number;
}

const props = defineProps<{
    data: { data: RekapKesesuaian[]; total: number; links: any[] };
...
watch([tahun, lembaga_id], () => {
    ...
    searchTimeout = setTimeout(() => {
        router.get('/auditor/rekap-kesesuaian', {
            tahun: tahun.value,
...
<template>
    <Head title="Rekapitulasi Daftar Kesesuaian" />
    <div class="space-y-6 p-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">Rekapitulasi Daftar Kesesuaian</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Rekap jumlah kesesuaian per auditee berdasarkan periode</p>
            </div>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-blue-50 text-blue-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-300">Jumlah Kesesuaian</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    >
                        ...
                        <td class="px-4 py-3">
                            <span v-if="item.jumlah_kesesuaian === 0" class="text-green-600 text-sm font-medium dark:text-green-400">
                                Tidak ada kesesuaian
                            </span>
                            <span v-else class="inline-flex items-center rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-2.5 py-0.5 text-xs font-semibold">
                                {{ item.jumlah_kesesuaian }} Kesesuaian
                            </span>
                        </td>
                    </tr>
                </tbody>
